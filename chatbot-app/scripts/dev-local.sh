...
ENV_OVERRIDE_PATH=${ENV_OVERRIDE_PATH:-"${REPO_ROOT}/env/.env.local-dev"}
...
PORT=${API_PORT} API_PORT=${API_PORT} npm --prefix apps/api run dev &
API_PID=$!

PORT=${WEB_PORT} WEB_PORT=${WEB_PORT} npm --prefix apps/web run dev &
WEB_PID=$!

trap 'kill -9 ${API_PID} ${WEB_PID}' EXIT
wait
